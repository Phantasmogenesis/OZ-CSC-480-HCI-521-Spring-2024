...
public class AdopterService extends AbstractService<Adopter> {
    ...
    // @APIResponse(responseCode = "400", description = "Invalid name or
    // configuration"),
    ...
    public Response retrieve(@QueryParam(value = "id") String id) {
        MongoCollection<Adopter> adopters = db.getCollection("Adopters", Adopter.class);
        var adopter = adopters.find(eq("_id", id)).first();
    ...
    public Response update(Adopter adopter) {
        ...
        UpdateResult result = adopters
                .replaceOne(and(eq("_id", adopter.getName()), eq("password", adopter.getEmailAddress())), newAdopter);
    ...
    public Response login(Adopter entry) {
        ...
        var record = adopters.find(and(eq("_id", entry.getName()), eq("password", entry.getEmailAddress()))).first();
